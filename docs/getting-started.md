...
# 6.（可选，AI 协作时）归档后做任务级规范沉淀 → 能力终点
```

⚠️ `toolkit init` 为 1.7.0 新增；旧版本请手工建 `.tasks/active/{YYYYMM}/` 目录结构。

> 不想把 `.tasks/` 放项目里？配置 `"tasks": { "dir": "../my-tasks-repo" }` 指向独立仓库（或每次用 `--dir` 传参），详见[配置参考](./config#tasks-任务目录与导入列映射)。
...
- 会话收尾说「会话要结束了，把结论记下来 / 今天先到这」→ AI 全量回放本会话 → 先给你核对任务清单 → 逐条落盘归档 + 规范沉淀（1.7.0 新增技能，1.8.0 全量沉淀）
