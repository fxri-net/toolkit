...
- **方案落盘与会话记忆**（依赖 `@fxri/toolkit` 及其 fxri-* skills，随 agent 自动加载；**细节一律以已加载的最新 skill 为准**，本规则只保留稳定纪律）：
  ...
  - 稳定纪律（不随版本变）：
    ...
    - 归档 + 规范沉淀即流程终点；提交、发版、推送不是必经步骤，需用户明确
    - 若产生代码变更需提交：先归档与沉淀、后提交，归档文件与代码变更落在同一 git 提交
...
| 方案落盘与会话记忆 | 把确认的方案、会话收尾结论变成 `.tasks/` 记录，人离开会话后决策仍可追溯；**可变细节收敛进 fxri-* skill（随 `skills update` 自动更新），全局规则只留稳定纪律**——因此能力升级后无需再手动同步本规则全文，避免「工具升了、规则没跟」的疏漏；执行顺序先项目级后全局；无任务区先 `toolkit init`；质量门不通过即修正；归档 + 沉淀为流程终点，提交代码则与任务记录同批 |
