...
- **方案落盘**（可选，依赖 @fxri/toolkit）：
  ...
  - 可用时：
    ...
    4. 归档即收尾：任务完成后标记 `status: 已完成` 并填写 `completed` 完成时间，执行 `toolkit tasks archive` 归档，强制约束到此为止；提交、发版、推送不是必经步骤，按用户规则约定执行，若提交代码则先归档、后提交，归档文件与代码变更落在同一 git 提交
...
| 方案落盘 | 把确认的方案变成 `.tasks/` 任务文件，人离开会话后决策仍可追溯；「先查后写」防多会话重复建档；强制约束到归档为止，若提交代码则归档文件与代码变更同批，保证任务记录和代码变更可对应 |
...
⚠️ **AI 行为约束**：安装与升级命令由用户本人执行，AI 不得自行执行全局安装/升级（`-g` 安装、skills 安装更新均属此列）；看到版本提示时提醒用户升级即可。
