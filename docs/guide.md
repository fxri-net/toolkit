...
conventions.md 可按项目规则体系选形态：
- **条文式**（无其他规则文件的项目）：存条文全文，作为项目唯一规范载体
- **溯源索引式**（已用 AGENTS / 全局规则的项目）：不复制条文，只记「确立了什么、谁确立的、条文去哪看」，内容以各单一事实源（skills / AGENTS / 全局规则）最新版为准——两种形态 skill 的写入/读取机制均支持
