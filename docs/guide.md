...
| 技能 | 版本 | 用途 |
| --- | --- | --- |
| `fxri-plan-to-task` | 1.3.1 | 方案落盘：建档评估（先查后写）→ 建档 → 校验 → 归档 → 任务级规范沉淀（能力终点）；规范载体迁移 |
| `fxri-release-changelog` | 1.1.0 | changesets 发版与多语言 CHANGELOG 维护 |
| `fxri-session-recap` | 1.2.1 | 会话收尾全量沉淀 + 规范沉淀 / 新会话三层恢复 / 历史任务时间批量修正 |
