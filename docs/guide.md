...
1. **动手前置建档评估（先查后写）**：动手改仓库文件前先 `toolkit tasks` 查 active 总览、核对 archive——active 有同主题更新原文件；archive 有同主题且无增量则不重复建档，有增量（延续/扩展）重新建档并标注来源链；均无则建档后再动手。归档任务不可变，不向已归档块追加内容
...
| 技能 | 版本 | 用途 |
| --- | --- | --- |
| `fxri-plan-to-task` | 1.2.0 | 方案落盘：建档评估（先查后写）→ 建档 → 校验 → 归档 → 任务级规范沉淀（能力终点）；规范载体迁移 |
| `fxri-release-changelog` | 1.0.12 | changesets 发版与多语言 CHANGELOG 维护 |
| `fxri-session-recap` | 1.2.0 | 会话收尾全量沉淀 + 规范沉淀 / 新会话三层恢复 / 历史任务时间批量修正 |
