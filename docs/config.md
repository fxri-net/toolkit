...
不同项目对脱敏、告警、语言、导入列映射的需求不同；`.toolkitrc.json` 让这些定制随仓库走、团队共享。个人偏好（关升级提示等）不必污染项目配置，放 `~/.toolkitrc.json` 全局文件即可，项目配置会按段覆盖它。

## 查找规则

- **项目配置**：从 `process.cwd()` **向上逐级**查找最近的 `.toolkitrc.json`，找到即用（支持在 monorepo 子目录运行，读最近一份）
- **全局配置**：同时读取 `~/.toolkitrc.json`——放个人偏好（关升级提示、个人脱敏规则、自定义语言表等），不进项目仓库、不随 git 分发，换机器需手动迁移
- 两层按**配置段**合并：项目配置出现的段**整体覆盖**全局同名段（非字段级深合并：项目写了某段，该段内未写的子字段也不会落到全局），项目未配的段取全局
- 覆盖链：CLI `--flag` > 环境变量 > 项目配置 > 全局配置 > 默认值
- 某级文件存在但 JSON 非法：跳过该级继续向上，都不合法视为无配置；全局文件非法同样视为无全局配置，不报错
...
CLI 每次命令执行末尾会异步查询 npm registry 最新版本（1 秒超时，离线/内网/超时静默失败），检测到新版本时向 **stderr** 输出一行升级提示；查询结果在系统临时目录缓存 24 小时，避免重复请求。关闭方式二选一：环境变量 `FX_NO_UPDATE_CHECK` 设为真值（`0/false/off/no` 视为未关闭，其余视为关闭），或本配置项设为 `false`。该检查不阻塞命令、不影响退出码。
