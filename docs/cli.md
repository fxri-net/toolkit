...
| 子命令 | 行为 |
| --- | --- |
| `archive` | 将 `status` 为 `已完成`/`已放弃` 且带 `completed` 的任务按完成日期聚合归档；排他锁防并发；缺 `completed` 的终结态任务跳过并提示；完成时间晚于当前系统时间时软告警（时间源疑似错误） |
| `check` | 校验 active：frontmatter 合法性、owner/created/命名规范、重名、`depends_on` 闭环、未闭合待办与 `- [ ]`；completed 晚于当前系统时间软告警；游离于 `active/` 层级外的日期前缀文件软告警 |
| `normalize` | 检查归档块：元数据四字段完整性、疑似任务块、完成时间与归档日期漂移、完成时间晚于当前系统时间（仅报告，不自动改值）、降序排序、月份目录归属；`--fix` 自动补齐/迁移/重排；`--fix` 与 `--check` 互斥 |
