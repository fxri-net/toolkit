...
| `check` | 校验 active：frontmatter 合法性、owner/created/命名规范、重名、`depends_on` 闭环、未闭合待办与 `- [ ]`；completed 晚于当前系统时间或恰为零点整（疑似只填日期被补零）软告警；游离于 `active/` 层级外的日期前缀文件软告警 |
| `normalize` | 检查归档块：元数据四字段完整性、疑似任务块、完成时间与归档日期漂移、完成时间晚于当前系统时间或恰为零点整（仅报告，不自动改值）、降序排序、月份目录归属；`--fix` 自动补齐/迁移/重排；`--fix` 与 `--check` 互斥 |
