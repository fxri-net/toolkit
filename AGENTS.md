...
2. **文档**：`docs/` 逐篇过触达面（README、getting-started、handbook、guide、faq、cli、api、ai-rules），凡触达处措辞必须与实际行为严格一致，包括「不自动修复」「需人工确认」等限制细节；确认无触达的内容须在对话中明确说明，不得静默跳过。⚠️ **时点约束**：触达面核对在**变更方案确认时**即按此清单逐篇走查、将结论列入任务文件「影响范围」节（每篇标注 已同步 / 无触达及理由），不延迟到归档时才复核；涉及术语或措辞变更时，先 grep 全仓库定位旧措辞触达点再列清单。
