...
2. **L1 工作层精读**：逐文件读 `.tasks/active/` 全部任务（决策、进展、阻塞）
3. **L2 近窗精读**：读最近归档。范围边界：归档月份 ≤ 6 个月或归档块 ≤ 200 条时精读全文；**超阈值降级为统计**（按月/负责人汇总 + 近期明细），防止历史膨胀撑爆上下文
4. **按需拉取**：更早历史不主动展开，在 L0 索引可见；用户点名、或 AI 判断相关时用 `--owner`/`--date` 过滤检索；用户明确说「全部恢复」才全量列出
5. **规范核对与提炼**：读 conventions.md（存在时）纳入现场；对照近期任务按「规范提炼子流程」提议增改——⚠️ 恢复主体只读，写 conventions.md 是用户确认后的独立动作
6. **输出**：全部任务索引摘要 → 进行中/阻塞任务 → 最近完成 → 未完事项 → 规范现场 → **建议的下一步**
