...
> 本技能版本 1.2.2（随 @fxri/toolkit 同批分发）。被问版本时即报此值——不读磁盘、不跑 CLI：报出的值就是本会话上下文里已加载内容的版本，可与 `toolkit skills status` 打印的磁盘基准值对照，不一致即说明会话上下文已过期，开新会话即可。
...
```
任务终结 → 归档 → 规范沉淀（硬终点） → 验收 → 提交/发版/推送（可选，需用户确认）
```

- 归档是中间步骤，**规范沉淀才是流程终点**
- 沉淀时机固定：归档完成后、任何收尾动作（验收 / 提交 / 发版 / 推送）前，规范与归档文件同批落盘
- ⚠️ **验收须晚于归档**：归档会改写 `.tasks/`，且归档本身不做阻断式校验——早于归档跑验收就覆盖不到终态，归档产物等于无人校验；归档块「完成情况」只记改动事实与所用验证命令，不逐条抄写验收输出
- 提交、发版、推送**不是必经步骤**，是否执行取决于用户全局/个人/项目规则；若提交代码：先归档、后跑项目验收命令、再提交，归档文件与代码变更落在同一 git 提交
- **动作不算、改动才算**：收尾动作（提交、发版、推送、部署）本身及其固有产物（版本号、更新日志、标签）不构成任务、不单独建档；其所依据的改动须有归档记录，缺则补档

### R3 规范落点：`.tasks/conventions/` 载体（两级沉淀同一载体）

- 唯一落点 `.tasks/conventions/`（纯 Markdown 随 git 走）：`index.md` 唯一入口（端清单 + 索引表 + 演进记录），`common.md` 承载所有端通用条文，`<端名>.md` 承载各端专属条文；每条规范带来源任务与日期，可审计
...
- **内容修订 vs 形态迁移**：旧条目过时走「演进记录留痕 + index 行更新 + 状态置 `已废弃`」（不删行）；使用者项目存旧单文件 `conventions.md` 且要求迁移时走**迁移三段式**（建目录搬原文 → 逐条提归属建议 → 用户确认后拆分册），可中断不丢内容
- ⚠️ 兼容读：先找 `conventions/index.md`，不存在再看旧单文件 `conventions.md`（提示可迁移）；两者并存时以目录形态为准
- 完整细则（载体结构、index 表列定义、两种形态、迁移步骤、自查清单）见 `../fxri-plan-to-task/references/conventions-spec.md`；fxri-plan-to-task 未一并安装、该兄弟路径不存在时，改读包内真源——`toolkit skills status` 打印的「技能源」目录下 `fxri-plan-to-task/references/conventions-spec.md`

### 规范提炼子流程（三模式共用）

1. 对照 conventions 载体收集候选：会话中用户明示的规则、与已有规范重复印证的决策、跨任务重复模式
2. 输出候选清单（每条含来源任务、建议归属与理由），交用户确认
3. 用户确认后写入载体：`index.md` 补索引行（归属 / 状态 / 确立来源任务），条文归位到 `common.md` 或命中的端分册
4. 无候选时完全静默，不强加流程
...
3. **逐条落盘**（先查后写；格式与目录规范见 `../fxri-plan-to-task/references/task-spec.md`；fxri-plan-to-task 未一并安装、该兄弟路径不存在时，改读包内真源——`toolkit skills status` 打印的「技能源」目录下 `fxri-plan-to-task/references/task-spec.md`）：
   ...
   - 收尾动作（提交、发版、推送、部署）本身及其固有产物（版本号、更新日志、标签）不单独成档；其所依据的改动缺归档记录时须补档
4. **归档**：终结态任务按 task-spec「手工归档步骤」执行（合并归档块 → 降序重排 → 写回 → 删源文件 → 清理空月份目录），或 `toolkit tasks archive` 自动化
5. **规范沉淀（终点）**：按「规范提炼子流程」执行会话级提炼，写入 conventions 载体
6. **收尾边界**：沉淀即本技能流程终点。提交、发版、推送不是必经步骤，是否执行取决于用户规则；⚠️ 验收须晚于归档——若提交代码，先归档、再跑项目验收命令、后提交，任务归档文件 + conventions 载体与本次代码变更落在**同一个 git 提交**；提交前向用户展示待提交内容供确认
...
5. **规范核对与提炼**：读 conventions 载体（存在时）纳入现场——常驻只读 `index.md`，再按任务 `scope` 命中加载 `common.md` 与端分册；对照近期任务按「规范提炼子流程」提议增改——⚠️ 恢复主体只读，写载体是用户确认后的独立动作
...
5. **规范观察**：批量回读历史任务时发现的重复模式，按「规范提炼子流程」列为候选（确认后写入载体，与修正动作同批）
...
| 新会话仍说「找不到上下文」 | 先查磁盘文件是否存在、是否落在正确月份目录、conventions 载体是否同目录；跨环境场景（换机 / 工作区被 git 清理）才需查是否已 git 提交 |
