...
> 本技能版本 1.2.1（随 @fxri/toolkit 同批分发）。被问版本时即报此值——不读磁盘、不跑 CLI：报出的值就是本会话上下文里已加载内容的版本，可与 `toolkit skills status` 打印的磁盘基准值对照，不一致即说明会话上下文已过期，开新会话即可。
...
6. **收尾边界**：沉淀即本技能流程终点。提交、发版、推送不是必经步骤，是否执行取决于用户规则；若提交代码，先归档、后提交，任务归档文件 + conventions 载体与本次代码变更落在**同一个 git 提交**；提交前向用户展示待提交内容供确认
